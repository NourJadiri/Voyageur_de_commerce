...
On commence par créer un graphe de 10 noeuds, auquels on associe des coordonnées qui suivent une loi uniforme $U[0,1]$.
```{r, echo=TRUE}
...
couts
```
Ensuite, on effectue un premier test pour calculer un chemin pour le voyageur de commerce. La méthode retenue ici est la méthode nearest, qui consiste à choisir le noeud le plus proche du noeud courant.
```{r, echo=TRUE}
